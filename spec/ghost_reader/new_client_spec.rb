...
describe GhostReader::NewClient do

  context 'on class level' do
    it 'should nicely initialize' do
      GhostReader::NewClient.new.should be_an_instance_of(GhostReader::NewClient)
    end
  end

  context 'a initialized client' do

    let(:client) { GhostReader::NewClient.new(:api_key => 'some+api_key') }
